...
const getRandomCell = () => {
    while(true){
        ...
        let cell = getFirstOpenCellForColumn(x);
        if(cell !== null){
            return cell;
        }
    }
...
const handleCellClick = (e) => {
  ...
  // computer play 
  if(gameIsLive){
    const randCell = getRandomCell();

    if (!randCell) return;
  
    randCell.classList.add(yellowIsNext ? 'yellow' : 'red');
    checkStatusOfGame(randCell);
